...
class Endpoint:
    def __init__(self,url_endpoint=None,ontology=None,search_type="classes"):
        self.url_endpoint = url_endpoint
        self.ontology = ontology
        print("Ontology is", self.ontology)
        if ontology:
            self.path_labels = f"sparql/temp/{ontology}_{search_type}/labels.obj"
            self.path_counts = f"sparql/temp/{ontology}_{search_type}/counts.obj"
        else:
    ...
    def run_sparql(self,query):
        try:
            #print("Sending to", self.url_endpoint)
            result_set = []
                ...
                for result in results["results"]["bindings"]:
                    ...
                    for var in result:
                        ...
                        if 'datatype' in result[var]:
                            if str(result[var]["datatype"])=="http://localhost:8894/null":
                                if value.lower() == "false" or value.lower() == "true":
    ...
    def listTerms(self,language=None,limit=10000):
        if self.ontology:
            ...
            FILTER (REGEX(STR(?term),"https://cenguix.github.io/Text2KGBench/ont_(0?[1-9]|1[0-9]|2[0-5])_{self.ontology}.+", "i"))
            OPTIONAL{{
        ...
        if self.search_type == "properties":
            query = query.replace("?term a owl:Class.", "?term a owl:ObjectProperty.")
            #print("REPLACING")
            query = query.replace("""{
                        SELECT ?term (COUNT(*) as ?qtd) WHERE{ 
                            [] [] ?term.
    ...
    def uri_to_label(self, uri):
        ...
        if ".owl " in raw_label:
            raw_label = raw_label.split(".owl ")[1]
    ...
    def get_labels_(self,uri):
        ...
        if self.ontology:
            ...
            FILTER(STRSTARTS(STR(<{uri}>),"https://cenguix.github.io/Text2KGBench/ont_(0?[1-9]|1[0-9]|2[0-5])_{self.ontology}.+"))
    ...
    def get_seealso(self,uri):
        ...
        PREFIX dbo: <http://dbpedia.org/ontology/>
        PREFIX dbp: <http://dbpedia.org/property/>
        SELECT DISTINCT ?other ?property WHERE{{
            <{uri}> ?property ?other.
            FILTER(
        ...
        descriptions = []
        for result in results:
            descriptions.append([result["?other"],result["?property"]])
        label_from_uri = self.uri_to_label(uri)
        if not label_from_uri in descriptions:
            descriptions.append([label_from_uri,"URI"])
        return descriptions
    ...
    def get_description(self,uri):
        ...
        SELECT DISTINCT ?description ?property WHERE{{
            <{uri}> ?property ?description.
            FILTER(
                ?property = terms:description ||
        ...
        descriptions = []
        for result in results:
            descriptions.append([result["?description"],result["?property"]])
        label_from_uri = self.uri_to_label(uri)
        if not label_from_uri in descriptions:
            descriptions.append([label_from_uri,"URI"])
        return descriptions
    ...
    def get_superclasses(self, uri):
        ...
        PREFIX dbo: <http://dbpedia.org/ontology/>
        PREFIX dbp: <http://dbpedia.org/property/>
        SELECT DISTINCT ?superclass ?property WHERE{{
            <{uri}> ?property ?superclass.
            FILTER(
        ...
        descriptions = []
        for result in results:
            descriptions.append([result["?superclass"],result["?property"]])
        label_from_uri = self.uri_to_label(uri)
        if not label_from_uri in descriptions:
            descriptions.append([label_from_uri,"URI"])
        return descriptions
    ...
    def get_subclasses(self, uri):
        ...
        PREFIX dbo: <http://dbpedia.org/ontology/>
        PREFIX dbp: <http://dbpedia.org/property/>
        SELECT DISTINCT ?subclass ?property WHERE{{
        ...
        descriptions = []
        for result in results:
            descriptions.append([result["?subclass"],result["?property"]])
        label_from_uri = self.uri_to_label(uri)
        if not label_from_uri in descriptions:
            descriptions.append([label_from_uri,"URI"])
        return descriptions
